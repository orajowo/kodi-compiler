...
# Ensure tmp is removed on exit (no exit inside this trap)
...
# Include executables and files matching *.so*; handle arbitrary filenames safely (print0)
mapfile -d '' -t candidates < <(find "${TMP_UNPACK}${PREFIX}" \( -type f -perm /111 -o -name '*.so*' \) -print0)
...
for bin in "${candidates[@]}"; do
  # dpkg-shlibdeps may error for some binaries; capture stdout (non-fatal)
  echo "Analyzing: ${bin}"
  out="$(dpkg-shlibdeps -O --ignore-missing-info "${bin}" 2>/dev/null || true)"
  ...
  if [[ "${out}" =~ =(.*) ]]; then
    ...
    for p in "${parts[@]}"; do
      ...
      # skip empty
      [ -z "${p}" ] && continue
      # append if not already present (exact match)
      if ! printf '%s\n' "${deps_set}" | grep -F -x -q "${p}"; then
        deps_set="${deps_set}
${p}"
      fi
    done
...
Priority: optional
Architecture: %ARCH%
Maintainer: Kodi Packager <orajowo@github.com>
Homepage: https://kodi.tv/
Standards-Version: 4.6.0

Depends: \${misc:Depends}, ${deps_all}
Recommends: va-driver-all
Suggests: kodi-pvr-iptvsimple, kodi-inputstream-adaptive, kodi-inputstream-rtmp, kodi-inputstream-ffmpeg-direct
